...
export function AppShell({ children, onLogOut, loggingOut = false }: { children: ReactNode; onLogOut?: () => void; loggingOut?: boolean }) {
  return <div className="grid min-h-svh grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)]">
    <aside className="hidden border-r bg-sidebar p-5 md:flex md:flex-col">
      ...
      <div className="mt-auto flex flex-col gap-1"><button className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-muted"><CircleHelp aria-hidden="true" />Help</button><button className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-muted disabled:opacity-50" disabled={loggingOut} onClick={onLogOut}><LogOut aria-hidden="true" />Log out</button></div>
    </aside>
